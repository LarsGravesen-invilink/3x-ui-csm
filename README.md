...
**3X-UI CSM** — настраиваемый скрипт для VPS, который умеет подключается к одной или нескольким панелям **3x-UI**, собирать все выбранные VPN-ключи со всех или выбранных инбаундов, формировать единый файл подписки и настраивает его раздачу через **Nginx** по HTTP или HTTPS.

ОГРАНИЧЕНИЕ: Скрипт предназначен только для личного, частного использования и работает только с одним файлом подписки и генерирует одну ссылку на неё.
